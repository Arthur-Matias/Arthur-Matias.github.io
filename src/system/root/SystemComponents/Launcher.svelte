<script lang="ts">
    ...
    import { isLauncherOpen, isLoading, isTurnedOff, settingsTrayOpen } from '../../../consts/stores'

    let w: number,h: number;
    let toolbarHeight = 76;
    const shortcuts: AppProps[] = state.allApps
...
<div id="launcher" bind:offsetHeight={h} bind:offsetWidth={w} class={$isLauncherOpen?"open round":"close"}>
    <span></span>
    <span id="apps" class="round">
        <div class="all-apps">
            {#each shortcuts as shortcut}
                <Shortcut props={shortcut} onRun={closeLauncher} openAtfirstClick={true} />
            {/each}
        </div>
...
<style>
    ...
    #launcher.open{
        ...
        animation: open-anim .2s forwards;
        z-index: 2 !important;
    }
    #apps{
        width: 100%;
