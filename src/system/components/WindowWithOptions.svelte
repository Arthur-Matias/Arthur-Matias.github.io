<script lang="ts">
    ...
    // onMount(()=>{
    //     props.geometry.position.x = props.geometry.position.x + activeindex * 32
    //     props.geometry.position.y = props.geometry.position.y + activeindex * 32
    // })
...
<div 
    class="window {maximized?"maximized":""} {$minimizedApps.includes(props.id)?"minimized":""}"
...
>
    <div class="options {maximized?"":"round-left"}">
        {#each props.options as option}
            <div class="option-btn" on:click={option.run}>
                <IconSelector iconName={option.icon} />
            </div>
        {/each}
    </div>
    <div class="window-content {maximized?"":"round-right"}">
        <div on:dblclick={handleWindowHeaderDBClick} class={`header ${maximized?"maximized":"round-top-right"}`} >
            ...
            <div class="window-controls">
                ...
                <div class="window-btn maximize" on:click={maximize}>
                        {#if maximized}
                        <div class="btn-icon">
                            <svg class="scale9" viewBox="0 0 17 17" xmlns="http://www.w3.org/2000/svg">
                                <path d="M15.0059 5.00409H12.4259L15.7159 1.71409C15.9042 1.52579 16.01 1.27039 16.01 1.00409C16.01 0.73779 15.9042 0.482395 15.7159 0.294092C15.5276 0.105788 15.2722 0 15.0059 0C14.7396 0 14.4842 0.105788 14.2959 0.294092L11.0059 3.57409V1.00409C11.0059 0.738875 10.9005 0.484522 10.713 0.296985C10.5254 0.109449 10.2711 0.00409174 10.0059 0.00409174C9.74064 0.00409174 9.48629 0.109449 9.29875 0.296985C9.11122 0.484522 9.00586 0.738875 9.00586 1.00409V6.00409C9.00586 6.26931 9.11122 6.52366 9.29875 6.7112C9.48629 6.89873 9.74064 7.00409 10.0059 7.00409H15.0059C15.2711 7.00409 15.5254 6.89873 15.713 6.7112C15.9005 6.52366 16.0059 6.26931 16.0059 6.00409C16.0059 5.73888 15.9005 5.48452 15.713 5.29699C15.5254 5.10945 15.2711 5.00409 15.0059 5.00409Z" fill="var(--bg-dark)"/>
                                <path d="M6.0058 9.00391H1.0058C0.740582 9.00391 0.486228 9.10926 0.298692 9.2968C0.111155 9.48434 0.00579834 9.73869 0.00579834 10.0039C0.00579834 10.2691 0.111155 10.5235 0.298692 10.711C0.486228 10.8985 0.740582 11.0039 1.0058 11.0039H3.5758L0.295798 14.2939C0.20207 14.3869 0.127676 14.4975 0.0769072 14.6193C0.0261385 14.7412 0 14.8719 0 15.0039C0 15.1359 0.0261385 15.2666 0.0769072 15.3885C0.127676 15.5103 0.20207 15.6209 0.295798 15.7139C0.388761 15.8076 0.499362 15.882 0.621222 15.9328C0.743081 15.9836 0.873786 16.0097 1.0058 16.0097C1.13781 16.0097 1.26852 15.9836 1.39038 15.9328C1.51223 15.882 1.62284 15.8076 1.7158 15.7139L5.0058 12.4239V15.0039C5.0058 15.2691 5.11116 15.5235 5.29869 15.711C5.48623 15.8986 5.74058 16.0039 6.0058 16.0039C6.27101 16.0039 6.52537 15.8986 6.7129 15.711C6.90044 15.5235 7.0058 15.2691 7.0058 15.0039V10.0039C7.0058 9.73869 6.90044 9.48434 6.7129 9.2968C6.52537 9.10926 6.27101 9.00391 6.0058 9.00391Z" fill="var(--bg-dark)"/>
                            </svg>
                        </div>
                        {/if}
                        {#if !maximized}
